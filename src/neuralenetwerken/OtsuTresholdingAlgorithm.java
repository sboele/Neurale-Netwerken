...
// Source from http://www.labbookpages.co.uk/software/imgProc/otsuThreshold.html
public class OtsuTresholdingAlgorithm {

	private int histData[];
	private int maxLevelValue;
	private int threshold;

	public OtsuTresholdingAlgorithm() {
	...
	public int[] doThreshold(int[] srcData) {
		int ptr;
		...
		int[] tresholdedImage = new int[srcData.length];
		// Apply threshold to create image with only black and white
		if (tresholdedImage != null) {
			ptr = 0;
			while (ptr < srcData.length) {
				tresholdedImage[ptr] = ((0xFF & srcData[ptr]) >= threshold) ? 1 : 0;
				ptr++;
			}
		}
